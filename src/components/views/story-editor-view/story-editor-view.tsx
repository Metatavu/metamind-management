...
import { Action, CanvasWidget, DefaultState, InputType } from "@projectstorm/react-canvas-core";
...
const StoryEditorView: React.FC<Props> = ({
  ...
  const translateToNode = (knot: Knot) => {
    const node = new CustomNodeModel({
      name: knot.name, id: knot.id, type: "custom-node"
    });
  ...
  const translateToHomeNode = (knot: Knot) => {
    const node = new HomeNodeModel({
      name: knot.name, id: knot.id, type: "home-node"
    });
    node.setPosition(knot.coordinates?.x || 200, knot.coordinates?.y || 200);
    return addNodeListeners(node);
  };
  ...
  const translateToGlobalNode = (knot: Knot) => {
    const node = new GlobalNodeModel({
      name: knot.name, id: knot.id, type: "global-node"
    });
    node.setPosition(knot.coordinates?.x || 200, knot.coordinates?.y || 200);
    return addNodeListeners(node);
  };
  ...
  const addInitialData = () => {
    ...
    const links = intents.reduce<CustomLinkModel[]>((_links, intent) => {
      const translatedLink = translateToLink(intent, nodes);
      return translatedLink ? [ ..._links, translatedLink ] : _links;
    }, []);
  ...
  React.useEffect(() => {
    ...
    if (node?.getID()) {
      engineRef.current.zoomToFitNodes({
        margin: 0, nodes: [node], maxZoom: 1
      });
  ...
  React.useEffect(() => {
    ...
    if (link) {
      ...
      if (sourceNode && targetNode) {
        engineRef.current.zoomToFitNodes({
          margin: 1, nodes: [ sourceNode, targetNode ], maxZoom: 1
        });
